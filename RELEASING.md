...
```sh
./scripts/bump.sh 0.6.3
git commit -am "release v0.6.3" && git push
```
...
```sh
cp target/Oxide-0.6.3.dmg target/Oxide-0.6.3-update.dmg
gh release create v0.6.3 target/Oxide-0.6.3.dmg target/Oxide-0.6.3-update.dmg \
  --title "Oxide v0.6.3" \
  --notes "$(sed -n '/^## \[0.6.3\]/,/^## \[/p' CHANGELOG.md | sed '1d;$d')"
```
...
git pull --ff-only && git fetch --tags    # HEAD is at (or just past) v0.6.3
...
```sh
git commit -am "aur: oxide-terminal-bin 0.6.3" && git push
cd packaging/aur/oxide-terminal-bin && makepkg --printsrcinfo > .SRCINFO
cp PKGBUILD .SRCINFO ~/aur/oxide-terminal-bin/
cd ~/aur/oxide-terminal-bin && git add -A && git commit -m "v0.6.3" && git push
```
...
- **The tag must be `v<Cargo.toml version>`** (e.g. `v0.6.3` for `0.6.3`) and
