...
```sh
./scripts/bump.sh 0.5.3
git commit -am "release v0.5.3" && git push
```
...
```sh
cp target/Oxide-0.5.3.dmg target/Oxide-0.5.3-update.dmg
gh release create v0.5.3 target/Oxide-0.5.3.dmg target/Oxide-0.5.3-update.dmg \
  --title "Oxide v0.5.3" \
  --notes "$(sed -n '/^## \[0.5.3\]/,/^## \[/p' CHANGELOG.md | sed '1d;$d')"
```
...
- **The tag must be `v<Cargo.toml version>`** (e.g. `v0.5.3` for `0.5.3`) and
