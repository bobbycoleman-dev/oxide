...
```sh
./scripts/bump.sh 0.5.4
git commit -am "release v0.5.4" && git push
```
...
```sh
cp target/Oxide-0.5.4.dmg target/Oxide-0.5.4-update.dmg
gh release create v0.5.4 target/Oxide-0.5.4.dmg target/Oxide-0.5.4-update.dmg \
  --title "Oxide v0.5.4" \
  --notes "$(sed -n '/^## \[0.5.4\]/,/^## \[/p' CHANGELOG.md | sed '1d;$d')"
```
...
- **The tag must be `v<Cargo.toml version>`** (e.g. `v0.5.4` for `0.5.4`) and
